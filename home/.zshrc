...
fy() {
    "$@" && notify-send "   Command beres ngab" "$*" || notify-send "❌ Command gagal ngab" "$*"
}

command_not_found_handler() {
    echo "'$1'?? paan njing!"
    # echo "💡 Try installing it with: sudo pacman -S $1"
    return 127
}
...
function chpwd() {
    case "$PWD" in
        "$HOME/Documents/repository")
            echo -e "\033[1;32m(Anda sekarang berada di directory repo  )\033[0m"
            ;;
        "$HOME/Documents/scripts")
            echo -e "\033[1;32m(Anda sekarang berada di directory script!  )\033[0m"
            ;;
    esac
}
